...
async function generateSchemas() {
  ...
  try {
    ...
    console.log(`📊 Found ${Object.keys(schemaInfos).length} schemas in OpenAPI spec`);
    console.log(`📋 Schema names: ${Object.keys(schemaInfos).join(', ')}\n`);
...
`;
    ...
    for (const [schemaName, schemaInfo] of Object.entries(schemaInfos)) {
      ...
      if (schemaInfo.properties && Object.keys(schemaInfo.properties).length > 0) {
        ...
        console.log(`⚠️  Skipping ${schemaName}: no properties found`);
      }
    }
